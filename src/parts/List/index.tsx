// src/parts/List/index.tsx

import { useState, useMemo } from 'react'
import { useNavigate } from 'react-router-dom'
import { MAIN_SKILL_KEYS, WEAPON_KEYS, type Character } from '../../domains/Character'
...
function List({ units, total }: { units: Character[], total: number }) {
  ...
  const sorted = useMemo((): Character[] => {
    if (sortKey === 'id') {
      ...
      return [...units].sort((a, b) => {
        const skillA = MAIN_SKILL_KEYS.indexOf(a.mainSkill.name)
        const skillB = MAIN_SKILL_KEYS.indexOf(b.mainSkill.name)
        return (skillA - skillB) * (sortDir.skill ?  -1 : 1)
      })
    } else if (sortKey === 'equip') {
      return [...units].sort((a, b) => {
        const equipA = WEAPON_KEYS.indexOf(a.weapon.name)
        const equipB = WEAPON_KEYS.indexOf(b.weapon.name)
        ...
        <tbody>
          {sorted.map((unit) => (
            <tr className="cursor-pointer" key={unit.id} onClick={() => navigate(`./${String(unit.id).padStart(2, '0')}/`)}>
              ...
              <td>{unit.weapon.name} / {unit.shield.size ? unit.shield.name : ''}{unit.shield.size ? ' /' : ''} {unit.armor.name}</td>

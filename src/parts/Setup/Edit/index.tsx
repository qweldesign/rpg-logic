...
function Edit() {
  ...
  const reducer: Reducer<State, Action> = (state, action) => {
    // 装備一覧を更新する関数
    const updateEquipList = (st: number, isSetTwoHanded: boolean) => {
      const weaponList = Object.entries(WEAPONS).filter(
        ([, weapon]) => !weapon.requiredST || weapon.requiredST <= st
      ) as [WeaponKey, Weapon][]
      const shieldList = isSetTwoHanded ? []
        :Object.entries(SHIELDS).filter(
          ([, shield]) => !shield.requiredST || shield.requiredST <= st
        ) as [ShieldKey, Shield][]
      const armorList = Object.entries(ARMORS).filter(
        ([, armor]) => !armor.requiredST || armor.requiredST <= st
      ) as [ArmorKey, Armor][]
    ...
    switch (action.type) {
      case 'INIT': {
        ...
        const isSetTwoHanded = equips.weapon.needsTwoHanded
      ...
      case 'STEP_PARAM': {
        ...
        const isSetTwoHanded = state.equips.weapon.needsTwoHanded
        
        // 装備一覧を更新
        const { weaponList, shieldList, armorList } = updateEquipList(nextParams.getLevel('筋力'), isSetTwoHanded)
      ...
      case 'SET_EQUIP': {
        ...
        const isSetTwoHanded = slot === 'weapon' && nextEquips.weapon.needsTwoHanded
      ...
      case 'RESET_EQUIPS': {
        ...
        const nextShield = state.equips.shield.name
        ...
        if (WEAPONS[nextWeapon].requiredST && WEAPONS[nextWeapon].requiredST > st) {
          resetedWeapon = state.prevEquips.weapon.name
        }
        if (SHIELDS[nextShield].requiredST && SHIELDS[nextShield].requiredST > st) {
          resetedShield = state.prevEquips.shield.name
        }
        if (ARMORS[nextArmor].requiredST && ARMORS[nextArmor].requiredST > st) {
  ...
  useEffect(() => {
    ...
    const checkEquipsByRequiredST = () => {
      ...
      const nextArmor = state.equips.armor.name
      return ((WEAPONS[nextWeapon].requiredST && WEAPONS[nextWeapon].requiredST > st)
        || (SHIELDS[nextShield].requiredST && SHIELDS[nextShield].requiredST > st)
        || (ARMORS[nextArmor].requiredST && ARMORS[nextArmor].requiredST > st)
      )
    }

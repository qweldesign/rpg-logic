...
function Combat() {
  ...
  const setup = () => {
    ...
    const { models: players, seed: savedSeed, usedRoster } = playerSetup(saveData)
    const seed = Math.ceil(savedSeed + Math.random() * 15) % 16
    const { models: enemies, rewardCp, rewardGold } = enemySetup(saveData, seed)
    return new State(players.concat(enemies), playLog, usedRoster, rewardCp, rewardGold)
  }
  ...
  useEffect(() => {
    ...
    if (result === 'win' && state && state.usedRoster && state.rewardCp && !state.rewardGranted) {
      ...
      saveData.saveGold(saveData.loadGold() + state.rewardGold)
    }
  }, [result])
                ...
                {result ? (
                  <div className="text-center">
                    ...
                    {result === 'win' && stateRef.current.rewardCp && (
                      <p className="mt-3 text-sm">CP +{stateRef.current.rewardCp} / 軍資金 +{stateRef.current.rewardGold}金</p>
                    )}

// src/domains/Combat/Mob/index.ts
...
// 名前, 基本パラメータ, 装備を引数に取って, 敵を生成
export function makeCombatEnemyModel(name: string, params: EnemyParams, equips: EnemyEquips, elements: SpellElement[] = []): UnitModel {
  return {
    name,
    maxHp: params.maxHp,
    level: params.level,
    dmgMod: params.dmgMod,
    dmgBuff: params.dmgBuff ?? 0,
    ev: params.ev,
    evBuff: params.evBuff ?? 0,
    pre: params.pre,
    mre: params.mre,
    equipments: new Equipments(...equips),
    elements: {
      blue: elements.includes('blue') ? params.level : 0,
      red: elements.includes('red') ? params.level : 0,
      green: elements.includes('green') ? params.level : 0
    }
  }
}
...
export function makeParamsByRank(params: EnemyParams, rank: number): EnemyParams {
  ...
  return {
    ...
    dmgBuff: params.dmgBuff ?? 0 + (rank + r3) % 2,
    ev: params.ev + Math.floor((rank + r2) / 2),
    evBuff: params.evBuff ?? 0 + (rank + r2) % 2,

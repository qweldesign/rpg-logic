...
│       │   ├── index.ts           # 戦闘ユニットを管理
│       │   ├── Health.ts          # 負傷状態を管理
│       │   ├── StatusBuff.ts      # バフ効果を管理
│       │   ├── StatusDebuff.ts    # デバフ効果を管理
│       │   ├── Attack.ts          # 攻撃パラメータを管理
│       │   └── Defense.ts         # 防御パラメータを管理
│       │
│       ├── Spells/
│       │   └── index.ts           # 魔法の管理
│       │
│       ├── AI/
│       │   ├── index.ts           # 敵AIを管理
│       │   ├── defender.ts        # 防御型AIを管理
│       │   ├── attacker.ts        # 攻撃型AIを管理
│       │   ├── supporter.ts       # 支援型AIを管理
│       │   ├── balanced.ts        # 万能型AIを管理
│       │   │
│       │   └── base/
│       │       ├── index.ts       # 基本形AIを管理
│       │       ├── blue.ts        # 青魔法AIを管理
...
- [x] 攻撃モデル/防御モデルを実装
- [x] 攻撃/防御/ダメージ判定を実装
- [x] 戦闘配置を司るクラスにターゲット取得関数を追加
- [x] 攻撃の基本実装
  - [x] ターゲット選択まで
  - [x] 実行前確認UIを追加
  - [x] ログ出力まで (ダメージ効果未実装)
  - [x] ダメージ効果を実装 (朦朧状態 / 転倒 / 気絶 / 死亡)
- [x] Hp・負傷に関する状態を管理するクラスを実装
- [x] 戦闘情報に戦闘状態を表示
- [x] 気絶の効果適用
- [x] 朦朧状態・転倒の効果適用と回復・立ち上がりを実装

### v0.5.0 戦闘行動の実装

- [x] 「全力防御」の実装と防御の試行回数制限の適用
- [x] 「牽制」を実装
- [x] 「準備」を実装
- [x] 「全力攻撃」を実装
- [x] 「待機」を破棄
...
- [x] 戦闘配置を司るクラスにに距離による修正の取得関数を追加
- [x] 戦闘ユニットのバフ/デバフを管理するクラスを実装
- [x] 魔法を定義 (機械的効果一覧を作成)
- [x] ユニットの使用できる魔法, 精神集中を管理するクラスを実装
- [x] 「集中」「魔法」のログ出力を実装
- [x] 魔法実行前確認UI (発動判定目標値表示) を追加
- [x] バフ・デバフ系を実装
- [x] 幻惑・狂戦士状態の効果を適用
- [x] 恐慌状態の自動行動を適用
- [x] 全体デバフ系を実装
